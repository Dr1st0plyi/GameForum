...
let AuthService = class AuthService {
    ...
    jwtService;
    steamService;
    constructor(usersService, prisma, jwtService, steamService) {
    ...
    async validateLocalUser(email, password) {
        ...
        if (user.role === client_1.UserRole.USER) {
            throw new common_1.UnauthorizedException('Please login with Steam');
        }
    ...
    async handleSteamLogin(profile) {
        if (!profile.id) {
            throw new common_1.UnauthorizedException('Invalid Steam profile');
        }
